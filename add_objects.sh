...
for i in $(seq 1 "$count"); do
  ...
  objects_url=$(get_objects_url "$HOKU_NETWORK")

  cmd="hoku -q --rpc-url $rpc_url bu add --object-api-url $objects_url --gas-limit 100000000 -b $mode --sequence $seq -a $bucket -k $key"
  cmd="$cmd $file_name"
  res=$($cmd 2>&1)
  echo "$res"
